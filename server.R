...
function(input, output, session) {
  ...
  fileName <- paste("StreamThermal"
                ...
                , "xlsx"
                , sep=".")
  ### this part is commented out because we cannot use XLConnect R package on cloud.gov
  ...
  ### this part is added to use "writexl" package
  
  Metric.Definitions <- read_excel(file.path(path.package("ContDataQC")
                                             ,"extdata"
                                             ,"StreamThermal_MetricList.xlsx")
                                   ,sheet = "MetricDefinitions"
                                   )
  
  worksheet_contents <- list(MetricDefinitions = Metric.Definitions
                             ,Notes = df.Notes
                             ,Groups = df.Groups
                             ,freq = processed$ST.freq
                             ,mag = processed$ST.mag
                             ,roc = processed$ST.roc
                             ,tim = processed$ST.tim
                             ,var = processed$ST.var)
  
  to_download$contents <- worksheet_contents
...
#   
  ...
  output$save_thermal <- downloadHandler(
    ...
    write_xlsx(to_download$contents,path=file) 
  ...
  observeEvent(input$display_IHA, {
   ...
   ## IHA parameters group 2: Magnitude of monthly water condition and include 12 parameters
    ...
    myDate <- format(Sys.Date(),"%Y%m%d")
    Notes.User <- Sys.getenv("USERNAME")
    myYr <- rownames(processed$IHA.group.1)[1]
    Notes.Names <- c("Dataset (SiteID)","IHA.Year","Analysis.Date (YYYYMMDD)","Analysis.User")
    ...
    ## this part is added to use "writexl" R package
    
    Notes.Summary <- as.data.frame.matrix(Notes.Summary)
    colnames(Notes.Summary) <- c("Date",input$parameter_name)
    
    IHA.group.1 <- as.data.frame(processed$IHA.group.1)
    IHA.group.1 <- cbind(rownames(IHA.group.1),data.frame(IHA.group.1,row.names = NULL))
    colnames(IHA.group.1)[1]<- "year"
    
    IHA.group.3 <- as.data.frame(processed$IHA.group.3)
    IHA.group.3 <- cbind(rownames(IHA.group.3),data.frame(IHA.group.3,row.names = NULL))
    colnames(IHA.group.3)[1]<- "year"
    
    IHA.group.4 <- as.data.frame(processed$IHA.group.4)
    IHA.group.4 <- cbind(rownames(IHA.group.4),data.frame(IHA.group.4,row.names = NULL))
    colnames(IHA.group.4)[1]<- "year"
    
    IHA.group.5 <- as.data.frame(processed$IHA.group.5)
    IHA.group.5 <- cbind(rownames(IHA.group.5),data.frame(IHA.group.5,row.names = NULL))
    colnames(IHA.group.5)[1]<- "year"
    
    IHA_worksheet_contents <- list(Notes = df.Notes
                               ,NotesSummary = Notes.Summary
                               ,Groups = df.Groups
                               ,Group1 = IHA.group.1
                               ,Group2 = as.data.frame(processed$IHA.group.2)
                               ,Group3 = IHA.group.3
                               ,Group4 = IHA.group.4
                               ,Group5 = IHA.group.5
                               )
    
    to_download$IHA_contents <- IHA_worksheet_contents
  ...
  output$save_IHA <- downloadHandler(
    ...
    content = function(file){
      ...
      write_xlsx(to_download$IHA_contents,path=file) 
